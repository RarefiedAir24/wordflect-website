...
export default function Profile() {
  ...
  const estimatedLevelProgress = (p: UserProfile) => {
    // Fake a smooth progress feel using points toward next level
    // Assumption: 1,000 points per level (client-only visualization)
    const perLevel = 1000;
  ...
  const aggregated = (p: UserProfile) => {
    ...
    // Ensure continuous days in range for nice sparkline
    ...
    // Filtered words for table/chips based on search
    const filtered = entries
      .filter(e => (search ? e.word.toLowerCase().includes(search.toLowerCase()) : true))
                ...
                {profile.profileImageUrl && profile.profileImageUrl.trim() !== '' ? (
                  <Image
                    ...
                    onError={() => {
                      ...
                      // Hide handled by surrounding UI
                    }}
                  />
                ) : (
...
// UI Subcomponents (pure CSS/HTML, no external deps)
